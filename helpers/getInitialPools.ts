...
const PEPI_MASTER_ABI = [
  ...
  },
  {
    ...
    name: "PEPI",
    outputs: [
      { internalType: "contract PepitaToken", name: "", type: "address" },
    ],
    stateMutability: "view",
    type: "function",
  },
  ...
  },
  {
    ...
    outputs: [
      ...
      { internalType: "uint256", name: "accChadPerShare", type: "uint256" },
...
export default getInitialPools;
...
async function getInitialPoolInfo(
  poolIndex: number,
  masterContract: ethers.Contract,
  provider
): Promise<InitialPool> {
  ...
  const fee = parseFloat(ethers.utils.formatUnits(poolInfo.depositFeeBP, 2));
  const allocPoints = parseFloat(
    ethers.utils.formatUnits(poolInfo.allocPoint, 0)
  );

  const pool = new InitialPool(
    poolToken,
    allocPoints,
    poolIndex,
    fee,
...
async function getToken(address, provider): Promise<Token | LpToken> {
  try {
  ...
  try {
    ...
    const erc20token = await getErc20(erc20, address);
